...
function Footer() {
  return (
    <footer className="relative section bg-zinc-800 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 via-sky-900/10 to-zinc-900 opacity-30 pointer-events-none" />
      ...
      <div className="container relative z-10 lg:grid lg:grid-cols-3 gap-10">
        ...
        <div className="flex flex-col">
          <h3 className="text-zinc-400 mb-4 font-semibold tracking-wide">
            Sitemap
          </h3>
          <ul className="space-y-2">
            {sitemap.map(({ label, href }, i) => (
              <li key={i}>
                <Link
                  to={href}
                  className="text-sm text-zinc-400 hover:text-sky-400 hover:underline transition-colors duration-300"
                >
                  {label}
                </Link>
              </li>
      ...
      {/* Bottom */}
      <div className="mt-12 pt-6 border-t border-zinc-700/50 flex flex-col lg:flex-row items-center justify-between text-sm text-zinc-500">
        <Link to="/" className="mb-2 lg:mb-0 flex items-center gap-2">
          <img
            src="/images/logo.svg"
            width={40}
            height={40}
